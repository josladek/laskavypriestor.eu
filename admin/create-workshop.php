...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($description) || empty($date) || 
        ...
        $error = 'Dátum workshopu musí byť v budúcnosti.';
        ...
        try {
            ...
            // Insert workshop - custom_instructor_name stored even with instructor_id
            $workshopId = db()->query("
                INSERT INTO workshops (
                    title, description, instructor_id, custom_instructor_name, date, time_start, time_end, 
                    duration_hours, capacity, price, is_free, notes, image_url,
            ...
            if ($workshopId) {
                setFlashMessage('Workshop bol úspešne vytvorený.', 'success');
                header('Location: ' . url('admin/workshops.php'));
                exit;
            } else {
                $error = 'Chyba pri vytváraní workshopu.';
            }
        } catch (Exception $e) {
            $error = 'Chyba pri vytváraní workshopu: ' . $e->getMessage();
        }
    }
}

$pageTitle = 'Vytvoriť workshop';
        ...
        <div class="col-md-8 mx-auto">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Vytvoriť nový workshop</h3>
                </div>
                <div class="card-body">
                    <?php if ($error): ?>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                                        ...
                                        <select class="form-select" id="instructor_id" name="instructor_id" required>
                                            ...
                                            <?php foreach ($instructors as $instructor): ?>
                                                <option value="<?= $instructor['id'] ?>" 
                                                        <?= (($_POST['instructor_id'] ?? '') == $instructor['id']) ? 'selected' : '' ?>>
                                                    <?= h($instructor['name']) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" class="form-control" id="capacity" name="capacity" 
                                       value="<?= h($_POST['capacity'] ?? '') ?>" min="1" required>
                            </div>
                            
                            <div class="col-md-4 mb-3">
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" id="is_free" name="is_free" value="1" 
                                           <?= isset($_POST['is_free']) && $_POST['is_free'] === '1' ? 'checked' : '' ?>
                                ...
                                <input type="number" class="form-control" id="price" name="price" 
                                       value="<?= h($_POST['price'] ?? '') ?>" step="0.01" min="0">
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="notes" name="notes" rows="3"><?= h($_POST['notes'] ?? '') ?></textarea>
                        </div>
                        
                        <div class="d-flex justify-content-between">
                            <a href="workshops.php" class="btn btn-secondary">Späť na zoznam</a>
                            <button type="submit" class="btn btn-primary">Vytvoriť workshop</button>
                        </div>
                    </form>
                    
                    <script>
                    function toggleInstructorName() {
                        const instructorSelect = document.getElementById('instructor_id');
                        const instructorNameField = document.getElementById('instructor_name');
                        
                        if (instructorSelect.value) {
                            instructorNameField.disabled = true;
                            instructorNameField.value = '';
                        } else {
                            instructorNameField.disabled = false;
                        }
                    }
